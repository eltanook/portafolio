...
Write-Host "  DB URL: $($dbCreds.internalConnectionString)" -ForegroundColor Gray
...
Write-Host "Enviando request a Render API..." -ForegroundColor Gray
...
try {
    ...
    Write-Host "  4. Configura permisos publicos para tus content types en Settings > Roles" -ForegroundColor White
    Write-Host ""
    Write-Host "Tip: El servicio gratuito se duerme despues de 15 min de inactividad." -ForegroundColor Yellow
    Write-Host "La primera request lo despertara (toma ~30 segundos)" -ForegroundColor Yellow
