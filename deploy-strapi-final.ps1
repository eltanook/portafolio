...
$db = $dbResponse.postgres

Write-Host "OK Base de datos: $($db.name)" -ForegroundColor Green
Write-Host "  Status: $($db.status)" -ForegroundColor Gray

# Obtener connection string
$connInfo = $db.connectionInfo
if ($connInfo) {
    $dbUrl = $connInfo.internalConnectionString
    Write-Host "  DB URL obtenida" -ForegroundColor Gray
} else {
    Write-Host "ERROR: No se pudo obtener connection string" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host ""
    Write-Host "Ver progreso del build en:" -ForegroundColor Yellow
    Write-Host "  $($result.service.dashboardUrl)" -ForegroundColor Cyan
