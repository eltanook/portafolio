...
while (-not $ready -and $attempts -lt 40) {
    ...
    try {
        ...
        Write-Host "  Esperando... (intento $attempts/40)" -ForegroundColor Gray
    }
}
...
# Obtener internal connection string de la DB
$internalConn = $dbInfo.internalConnectionString
$externalConn = $dbInfo.externalConnectionString

Write-Host "  DB Host: $($dbInfo.hostname)" -ForegroundColor Gray
Write-Host "  DB Port: $($dbInfo.port)" -ForegroundColor Gray
Write-Host "  DB Name: $($dbInfo.databaseName)" -ForegroundColor Gray
...
$serviceData = @{
    ...
    envVars = @(
        ...
        @{ key = "DATABASE_URL"; value = $internalConn }
...
try {
    ...
    if ($_.Exception.Response) {
        ...
        Write-Host "Respuesta completa: $responseBody" -ForegroundColor Red
    }
    exit 1
}
